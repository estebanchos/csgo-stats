...
export default function PlayersMultiselect() {
  ...
  const nonSelectedPlayers = players?.filter((player) => !selectedPeople?.some(selected => selected.id === player.id))
  ...
  return (
    ...
    <Combobox value={selectedPeople} onChange={setSelectedPeople} multiple>
      ...
      >
        <Combobox.Input
          className="min-w-[18rem] border-none py-2 pl-3 pr-10 text-sm leading-5 text-gray-900 focus:ring-0"
        />
