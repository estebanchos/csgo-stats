...
import { type TValueUpdate } from '~/pages/players';
...
export default function UpdatePlayerInfo({ isOpen, closeModal, fieldUpdate, refetchPlayers }: IProps) {
  ...
  function onSubmit(data: TForm) {
    if (fieldUpdate.field === 'name') {
      ...
      editName.mutate({
        playerId: fieldUpdate.playerId,
        name: data.updateValue,
      });
    }
            ...
            <div className='flex min-h-full items-center justify-center p-4 text-center'>
              ...
              >
                <Dialog.Panel className='w-full max-w-md transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all'>
                  ...
                  <form action='submit' onSubmit={handleSubmit((data) => onSubmit(data as TForm))}>
                    <div className='mt-2'>
                      <input
                        id='updateValue'
                        type='text'
                        inputMode='text'
                        className={`focus:gold-focus-ring mt-1 block w-full border px-4 py-2 text-sm font-normal leading-5 text-gray-700 placeholder-gray-700 ${
                          errors?.updateValue ? 'rounded-sm border-orange-600' : 'border-primary'
                        }`}
                    ...
                    <div className='mt-4 flex gap-x-4'>
                      <button
                        type='submit'
                        className='inline-flex w-full justify-center rounded-md border border-transparent bg-blue-100 px-4 py-2 text-sm font-medium text-gray-800 hover:bg-blue-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2'
                      >
                        Update
                      </button>
